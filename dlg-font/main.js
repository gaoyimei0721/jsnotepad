...
var $dlgFont = (function(){    
  var DOM=''
      ...
      +'      <input type="button" value="取消" class="btn-cancel"/>'       +'    </div>'
      +'  </div>'
      +'</div>';
  ...
  function show(){     
    var familys=['Agency FB', 'Algerian', 'Arial', 'Arial Rounded MT', 'Axure Handwriting', 'Bahnschrift', 'Baskerville Old Face', 'Bauhaus 93', 'Bell MT', 'Berlin Sans FB', 'Bernard MT', 'BlackAdder ITC'];        var styles=['常规', '斜体', '粗体', '粗偏斜体'];            
    var sizes=['8', '9', '10', '11', '12', '14', '16', '18', '20', '22', '24', '26', '28', '36', '48', '72'];                    
    $('body').append($dlg);                        
    $familyBox.append(initBox($familyInp,familys,'F'));                   $styleBox.append(initBox($styleInp,styles,'Y'));                      $sizeBox.append(initBox($sizeInp,sizes),'S');

    $dlg.find('.dialogbox').draggable({handle:$dlg.find('.titlebar')});                                      
    $btnCancel.click(function(){$dlg.remove();});                         $btnConfirm.click(function(){$dlg.remove();});       
